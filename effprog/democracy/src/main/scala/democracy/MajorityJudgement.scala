...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    grades.sortBy(grade => grade.ordinal).apply(grades.length/2)
end Grade
...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def elect(ballots: Seq[Ballot]): Candidate =
    ...
    // `Candidate`.
    val gradesPerCandidate: Map[Candidate, Seq[Grade]] = allGrades.groupMap((c, g) => c)((c, g) => g)
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      // median grade.
      val bestMedianGrade: Grade = gradesPerCandidate.values.filter(grades => grades.nonEmpty)
        .map(Grade.median).maxBy(_.ordinal)
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] =
        gradesPerCandidate.filter((c, grades) => grades.nonEmpty && Grade.median(grades) == bestMedianGrade)
      ...
      if bestCandidates.size == 1 then
        ...
        val bestCandidatesMinusOneMedianGrade: Map[Candidate, Seq[Grade]] =
          bestCandidates.map((c, grades) => (c, grades.diff(Seq(bestMedianGrade))))
