...
abstract class TweetSet extends TweetSetInterface {
  ...
  def findMostRetweeted(best: Tweet): Tweet
  ...
  def descendingByRetweet: TweetList = {
    ...
    def listAcc(set: TweetSet, list: TweetList): TweetList = {
      set match {
        case _: NonEmpty => {
            val best = set.mostRetweeted
            listAcc(set.remove(best), new Cons(best, list))}
        case _: Empty => list
      
      }
    }
    
    def reverse(list: TweetList, reversed: TweetList): TweetList = {
      if (list.isEmpty) reversed
      else reverse(list.tail, new Cons(list.head, reversed))
    }
    
    reverse(listAcc(this, Nil), Nil)
...
class Empty extends TweetSet {
  ...
  def findMostRetweeted(best: Tweet): Tweet = best
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def findMostRetweeted(best: Tweet): Tweet = {
    right.findMostRetweeted(left.findMostRetweeted(if (elem.retweets > best.retweets) elem else best))
  }
...
object GoogleVsApple {
   ...
   */
  lazy val united = googleTweets union appleTweets
  lazy val trending: TweetList = united.descendingByRetweet
}
