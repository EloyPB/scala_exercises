...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for p <- b.legalNeighbors yield (p(0), p(1) :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter((p: (Block, List[Move])) => !explored.contains(p(0)))
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] =
    val frontier = initial.flatMap((p: (Block, List[Move])) =>
      newNeighborsOnly(neighborsWithHistory(p(0), p(1)), explored))
    initial #::: from(frontier, explored ++ frontier.map(_(0)))
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter((p: (Block, List[Move])) => done(p(0)))
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.head(1).reverse
