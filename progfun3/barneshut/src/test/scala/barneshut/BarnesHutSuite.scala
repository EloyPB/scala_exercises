...
class BarnesHutSuite extends munit.FunSuite:
  ...
  test("Inserting a body on a Leaf with 1 body") {
    val b1 = Body(2f, 12f, 16f, 0f, 0f)
    val b2 = Body(3f, 13f, 18f, 0f, 0f)
    val quad = Leaf(10f, 20f, 5f, Seq(b1)).insert(b2)
    val quadType = quad match
      case f:Fork => {
        assert(f.ne.mass == 5f)
        "fork"
      }
      case _ => "other"
    assert(quadType == "fork")
  }
  ...
  test("Body.updated should consider a Fork as opaque if it is far away") {
    val b1 = Body(2f, 100f, 100f, 0f, 0f)
    val b2 = Body(2f, 11f, 19f, 0f, 0f)
    val b3 = Body(3f, 9f, 21f, 0f, 0f)
    val quad = Leaf(10f, 20f, 5f, Seq(b2)).insert(b3)
